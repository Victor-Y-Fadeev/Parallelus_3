...
{
	const float p = C / A + (B * B) / (A * A);
	const float q = 2 * B * B * B / (A * A * A) - 3 * (B * C) / (A * A) + D / A;
    const float Q1 = (p / 3)*(p / 3)*(p / 3) + (q / 2)*(q / 2);

    if (fabsf(Q1)<eps)
    {
        const float alpha = powf(- q / 2,1./3);
    ...
    {
        const float alpha = powf(- q / 2 + sqrtf(Q1),1./3);
        const float beta = powf(- q / 2 - sqrtf(Q1),1./3);
    ...
    else
    {
        ...
        const float ReAlpha = powf(Ralpha, 1./3) * cosf(Falpha);
        const float ImAlpha = powf(Ralpha, 1./3) * sinf(Falpha);

        const float Rbeta =  Ralpha;
        const float Fbeta =  atanf (-ImQ /( q * q / 4)) / 3;
        const float ReBeta = powf(Rbeta, 1./3) * cosf(Fbeta);
        const float ImBeta = powf(Rbeta, 1./3) * sinf(Fbeta);

        const float x1 = ReAlpha + ReBeta;
        const float x2 = - (ReAlpha + ReBeta) / 2 - ((ImAlpha - ImBeta)*sqrtf(3)/2);
        const float x3 = - (ReAlpha + ReBeta) / 2 + ((ImAlpha - ImBeta)*sqrtf(3)/2);
...
int num ( Vector *vector, SpecialPoint *sPoint)
{
    ...
    const float B1 = 2*a + b + c + alpha + beta;
    const float C1 = a + 2*beta + alpha;
    const float D1 = beta;
